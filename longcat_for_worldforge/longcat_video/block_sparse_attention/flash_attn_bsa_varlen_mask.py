...
def _attn_fwd_bsa_varlen_align(
    ...
    # load scales
    qk_scale = sm_scale
    qk_scale *= 1.44269504  # 1/ln2; multiply by 1/ln2 so exp2(x/ln2) == exp(x), exp2 is faster
    # load q: it will stay in SRAM throughout
    q = tl.load(Q_block_ptr)
    S = tl.load(block_indices_lens)
    for i in range(S):
        ...
        # update acc
        v = tl.load(V_block_ptr_i)
        acc = tl.dot(p.to(v.dtype), v, acc) # No division by se here, optimization from FlashAttention-2
        # update m_i and l_i
        # place this at the end of the loop to reduce register pressure: https://github.com/triton-lang/triton/commit/ee6abd9
        l_i = l_i * alpha + l_ij # current cumulative sum of exp
        m_i = m_ij
