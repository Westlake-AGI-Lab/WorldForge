...
class LongCatVideoPipeline:
    ...
    def get_timesteps_sigmas(self, sampling_steps: int, use_distill: bool=False):
        if use_distill:
            ...
            sigmas = torch.linspace(0.999, 0.000, sampling_steps)
            # sigmas = torch.linspace(1, 0.001, sampling_steps)
            # sigmas = torch.linspace(1, 0.000, sampling_step
    ...
    def generate_i2v(
        ...
        # Process video_ref and mask for guidance
        if video_ref is not None and guided:
            ...
            loguru.logger.info(f"Reference video processed: {video_ref.shape}")
        
        if mask is not None and guided:
            if isinstance(mask, np.ndarray):
                mask = torch.from_numpy(mask)
            
            # Ensure mask is [1, 1, T, H, W]
            if mask.dim() == 3:
            ...
            mask = mask.to(device)
            loguru.logger.info(f"Mask processed: {mask.shape}")
        
        # Initialize derivative history for auto-guidance
        ...
        with tqdm(total=len(timesteps), desc="Denoising") as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # Clear derivative history for current step
                self.scheduler.derivative_history = []
                ...
                for r in range(resample_steps if (guided and i < resample_round) else 1):
                    # Set resample mode and rollback step_index
                    if r > 0:
                        self.scheduler.set_resample_mode(True)
                        # Rollback step_index for resample (use same timestep t)
                        self.scheduler._step_index -= 1
                    else:
                        self.scheduler.set_resample_mode(False)
                    
                    # Always use the same timestep t for both initial and resample iterations
                    timestep_for_dit = t.expand(latents.shape[0])
                    timestep_for_dit = timestep_for_dit.to(device=latents.device, dtype=dit_dtype)

                    latent_model_input = torch.cat([latents] * 2) if self.do_classifier_free_guidance else latents
                    latent_model_input = latent_model_input.to(dit_dtype)

                    # If CFG, timestep also needs to be doubled to match latent_model_input batch size
                    if self.do_classifier_free_guidance:
                        timestep_for_dit = torch.cat([timestep_for_dit] * 2)

                    timestep = timestep_for_dit.unsqueeze(-1).repeat(1, latent_model_input.shape[2])
                    timestep[:, :1] = 0
                    ...
                    if self.do_classifier_free_guidance:
                        ...
                        positive = noise_pred_cond.reshape(B, -1)
                        negative = noise_pred_uncond.reshape(B, -1)
                        # Calculate the optimized scale (CFG-zero)
                        st_star = self.optimized_scale(positive, negative)
                        st_star = st_star.view(B, 1, 1, 1)
                    ...
                    scheduler_output = self.scheduler.step(
                        ...
                        video_ref=video_ref,
                        mask=mask,
                        guided=guided and i < guide_steps,
                        resampling=r > 0,
                        vae=self.vae,
                    ...
                    # Get pred_x0 for resampling
                    if hasattr(scheduler_output, 'pred_x0') and scheduler_output.pred_x0 is not None:
                        pred_x0 = scheduler_output.pred_x0
                    
                    # Break if beyond resample rounds
                    if i >= resample_round:
                        break
                    
                    # Re-noise for next resample iteration
                    if r < resample_steps - 1 and pred_x0 is not None:
                        # Generate noise using the same generator as initial latents
                        noise = torch.randn(pred_x0.shape, generator=generator).to(
                            device=pred_x0.device, dtype=pred_x0.dtype
                        )
                        
                        # Use current timestep t for re-noising
                        current_timestep = t.expand(pred_x0.shape[0]).to(device=latents.device)
                        
                        # Add noise using Euler's flow-matching forward process
                        latents_renoised = self.scheduler.add_noise(
                            pred_x0,
                            noise,
                            current_timestep,
                            use_resample_sigma=False  # Use standard sigma from timestep
                        )
                        latents[:, :, 1:] = latents_renoised
                
                # Reset resample mode
                self.scheduler.set_resample_mode(False)
                
                # Auto-Guidance (after resample loop, within resample_round)
                # Only apply if we have multiple noise predictions from resampling
                if i < resample_round and len(self.scheduler.derivative_history) > 1 and guided:
                    noise_pred_worse = self.scheduler.derivative_history[0]
                    noise_pred_good = self.scheduler.derivative_history[-1]
                    
                    # Compute cosine similarity
                    dot_product = torch.sum(noise_pred_good * noise_pred_worse, 
                                          dim=list(range(1, noise_pred_good.dim())), keepdim=True)
                    norm_good = torch.sqrt(torch.sum(noise_pred_good**2, 
                                                    dim=list(range(1, noise_pred_good.dim())), keepdim=True))
                    norm_worse = torch.sqrt(torch.sum(noise_pred_worse**2, 
                                                     dim=list(range(1, noise_pred_worse.dim())), keepdim=True))
                    ...
                    # Select omega based on step
                    current_omega = omega_resample if i >= guide_steps else omega
                    
                    # Cosine-improved auto-guidance
                    noise_pred_better = noise_pred_good + current_omega * sin_theta * (
                        noise_pred_good - magnitude_ratio * cos_theta * noise_pred_worse
                    )
                    
                    # Apply better prediction
                    self.scheduler._step_index -= 1
                    
                    better_output = self.scheduler.step(
                        noise_pred_better,
                        t,
                        latents[:, :, 1:],
                        guided=False,
                    ...
                    latents[:, :, 1:] = better_output.prev_sample
                else:
                    # No auto-guidance, use last scheduler output
                    if scheduler_output is not None:
                        latents[:, :, 1:] = scheduler_output.prev_sample
    ...
    def generate_refine(
        ...
        latent_up = retrieve_latents(self.vae.encode(video_UP), generator=generator)
        latent_up = self.normalize_latents(latent_up)
        noise = torch.randn(latent_up.shape, generator=generator, dtype=latent_up.dtype).to(device=latent_up.device)
        latent_up = (1 - t_thresh) * latent_up + t_thresh * noise
        del video_DOWN, video_UP, stage1_video, noise
        torch_gc()
